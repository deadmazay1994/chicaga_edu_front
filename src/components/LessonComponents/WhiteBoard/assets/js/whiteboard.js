...
export default {
  ...
  settings: {
    ...
    canvasWidth: 3000,
    canvasHeight: 2000,
    backgroundGridUrl: "/imgs/whiteboardBg.png"
  },
  loadWhiteboard: function(whiteboardContainer, newSettings) {
    ...
    this.settings["whiteboardId"] = this.settings["whiteboardId"].replace(
      /[^0-9a-z]/gi,
      ""
    );
    ...
    _this.backgroundGrid = $(
      "<div style=\"position: absolute; left:0px; top:0; opacity: 0.2; background-repeat: repeat; background-image:url('" +
        _this.settings["backgroundGridUrl"] +
    ...
    _this.svgContainer = $(
      '<svg style="position: absolute; top:0px; left:0px;" width="' +
        _this.settings.canvasWidth +
        '" height="' +
    ...
    _this.mouseOverlay = $(
      '<div style="cursor:none; position: absolute; left:0px; top:0; height: 100%; width: 100%;"></div>'
    );
    ...
    $(_this.mouseOverlay).on("mousemove touchmove", function(e) {
      ...
      window.requestAnimationFrame(function() {
        ...
        if (_this.tool === "eraser") {
          ...
          left = currX - _this.thickness / 2;
          top = currY - _this.thickness / 2;
          _this.ownCursor.css({ top: top + "px", left: left + "px" });
        } else if (_this.tool === "line") {
          if (svgLine) {
            if (shiftPressed) {
          ...
          if (svgRect) {
            ...
            if (shiftPressed) {
              ...
              x = currX < startCoords[0] ? currX : startCoords[0];
              y = currY < startCoords[1] ? currY : startCoords[1];
              svgRect.setAttribute("x", x);
              svgRect.setAttribute("y", y);
            }
      ...
      if (_this.tool === "line") {
        ...
        var imgDiv = $(
          '<div style="position:absolute; left:' +
            ...
            "px; width:" +
            width +
            "px; border: 2px dotted gray; overflow: hidden; height:" +
            height +
            'px;" cursor:move;">' +
            '<canvas style="cursor:move; position:absolute; top:0px; left:0px;" width="' +
            width +
            '" height="' +
            height +
            '"/>' +
        ...
        var dragOutOverlay = $(
          '<div class="dragOutOverlay" style="position:absolute; left:' +
            left +
            "px; top:" +
            top +
            "px; width:" +
        ...
        imgDiv.find(".xCanvasBtn").click(function() {
          ...
          if (_this.tool === "pen") _this.mouseOverlay.css({ cursor: "none" });
          else _this.mouseOverlay.css({ cursor: "crosshair" });
          imgDiv.remove();
    ...
    $(_this.mouseOverlay).on("mouseout", function() {
      if (_this.imgDragActive) {
        return;
      }
      _this.drawFlag = false;
    ...
    $(_this.mouseOverlay).on("mouseover", function() {
      if (_this.imgDragActive) {
        return;
      }
      if (!_this.mouseover) {
    ...
    $(document).on("keydown", function(e) {
      if (e.which == 17) {
        ...
        //Esc
        if (!_this.drawFlag) _this.svgContainer.empty();
    ...
    function getRoundedAngles(currX, currY) {
      ...
      if (angle45 % 90 == 0) {
        if (
          Math.abs(currX - startCoords[0]) > Math.abs(currY - startCoords[1])
        ) {
          currY = startCoords[1];
        } else {
          currX = startCoords[0];
  ...
  addImgToCanvasByUrl: function(url) {
    ...
    _this.mouseOverlay.css({ cursor: "default" });
    var imgDiv = $(
      '<div style="border: 2px dashed gray; position:absolute; left:200px; top:200px; min-width:160px; min-height:100px; cursor:move;">' +
  ...
  undoWhiteboard: function(username) {
    ...
    for (var i = _this.drawBuffer.length - 1; i >= 0; i--) {
      if (_this.drawBuffer[i]["username"] == username) {
        ...
        for (i = _this.drawBuffer.length - 1; i >= 0; i--) {
          if (
        ...
        if (content["event"] === "move") {
          if (
            ...
            _this.cursorContainer
              ...
              .css({ left: data[0] + "px", top: data[1] + "px" });
          } else {
  ...
  isRecRecCollision: function(rx1, ry1, rw1, rh1, rx2, ry2, rw2, rh2) {
    return (
      rx1 < rx2 + rw2 && rx1 + rw1 > rx2 && ry1 < ry2 + rh2 && rh1 + ry1 > ry2
    );
  },
